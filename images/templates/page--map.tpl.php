...
$cLogic = " AND TRUE ";
if ($cType == "LRS") {
  $cLogic = " AND (tx.name='LRS1' OR tx.name='LRS2')";
}
else {
  if ($cType > "") {
...
db_query($sql);
$query = db_select('statesidesearchfororgs', 'sl')
  ->extend('PagerDefault')
  ->extend('TableSort');
$query->fields("sl", [
  "nid",
...
if ($numlist > 0) {
  ...
  foreach ($rsList as $rs) {
    ...
    if (in_array(strtolower($rs->tag), ['lrs1', 'lrs2'])) {
    ...
    if ($rs->url) {
      $displayname = "<a href='http://" . $webPage . " ' target='_blank''>" . $orgname . "</a>";
    }
    $fullName = $orgname;
    $addressDetails = "";
    if (trim($address1) > "") {
...
<script type="text/javascript">
  ...
  function initialize() {
    ...
    var mapOptions = {
      zoom: myzoom, center: myLatlng, mapTypeId: google.maps.MapTypeId.ROADMAP
    }
  ...
  function initializePoint(LatLng, icon) {
    ...
    var marker = new google.maps.Marker({
      position: LatLng, map: map, icon: icon, title: 'Click to zoom in'
    });
...
<div id="map_canvas" style="display:none;">
    <div>
        ...
        <script src="<?php print drupal_get_path('theme', 'state7'); ?>js/jquery.js"
                type="text/javascript"></script>
        <script src="<?php print drupal_get_path('theme', 'state7'); ?>js/jquery.dropdown.js"
                type="text/javascript"></script>
        <script>
