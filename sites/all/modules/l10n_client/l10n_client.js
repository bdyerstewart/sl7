  ...
  Drupal.behaviors.l10nClient = {
    attach: function (context) {
      $('#l10n-client').once('l10n-client', function () {
        ...
        // If the selection changes, copy string values to the source and
        // target fields. Add class to indicate selected string in list widget.
        ...
        if ($.hotkeys) {
          ...
          $.hotkeys.add(Drupal.l10nClient.keys.clear, {
            target: '#l10n-client .string-search', type: 'keyup'
          }, function () {
        ...
        $l10nClientForm.submit(function () {
          ...
          $this.find('.edit-save').after('<div class="ajax-progress ajax-progress-throbber">' + '<div class="throbber">&nbsp;</div><div class="message">' + Drupal.t('Please wait...') + '</div></div>');

          $.ajax({
            type: "POST", url: $this.attr('action'), // Send source and target
                                                     // strings.
            data: {
              ...
              // Figure out the display of the new translation in the selection
              // list.
              ...
              // HTML tags will show up in the selector, but that is normal in
              // this case.
              ...
              if (newTranslationStripped.length > 81) {
                ...
                // We strip at 78 chars and add three dots, if the total length
                // is above 81.
