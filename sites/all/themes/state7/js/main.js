...
function resize_to_fit(outer, elem){
    return;
    var fontsize = jQuery(outer + " " + elem).css('font-size');
    var padding = jQuery(outer + " " + elem).css('padding-top');
    padding = parseFloat(padding);
    padding = Math.max(padding, 10)
    jQuery(outer + " " + elem).css('fontSize', parseFloat(fontsize) - 1);
    jQuery(outer + " " + elem).css('padding-top', (padding - 2) + "px");
    if(jQuery(outer + " " + elem).height() + padding >= jQuery(outer).height()){
        resize_to_fit(outer, elem);
    }
}
